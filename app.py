...
def fetch_smc_data(coin, period_label):
    ...
    hl_pos = []
    if lb:
        addrs = [r["ethAddress"] for r in lb.get("leaderboardRows", [])[:200] if r.get("ethAddress")]
        def fetch_hl(a):
...
if data:
    ...
    with c2: range_p = st.selectbox("표시 범위 %", [5, 10, 15, 20, 30], index=1)
    ...
    # Gap 탐지: 물량이 max의 10% 미만인 연속 구간
    for i in range(len(prices)-1):
        if (ladder[prices[i]]['L'] + ladder[prices[i]]['S']) < (max_wall * 0.1):
            gaps.append(prices[i])
    ...
    with a1:
        ...
        st.markdown(f"""
            <div class="analysis-card" style="border-top:4px solid {sfp_color};">
                <div class="sub-label">SFP Sweep Intensity</div>
                <div class="val-main" style="color:{sfp_color};">{sweep_intensity:.1f}%</div>
                <div style="font-size:13px; color:#aaa;">
                    {"🚨 <b>세력 매집 완료!</b> 반전 타점 발생" if sweep_intensity > 50 else "유동성 확보 패턴 분석 중..."}
                </div>
                <div style="margin-top:15px; font-size:12px;">
                    추천 대응: <span class="price-tag">{coin} {"숏(Short)" if px > (sum(prices)/len(prices) if prices else px) else "롱(Long)"} 진입 대기</span>
                </div>
            </div>
        """, unsafe_allow_html=True)

    with a2:
        gap_start = min(gaps) if gaps else 0
        gap_end = max(gaps) if gaps else 0
        st.markdown(f"""
            <div class="analysis-card" style="border-top:4px solid var(--cyan);">
                <div class="sub-label">Liquidity High-Speed Zone</div>
                <div class="val-main" style="color:var(--cyan);">${gap_end - gap_start:,.0f} GAP</div>
                <div style="font-size:13px; color:#aaa; margin-bottom:10px;">
                    변동성 폭발 구간: <span class="price-tag">${gap_start:,.0f} ~ ${gap_end:,.0f}</span>
                </div>
                <div style="font-size:11px; color:var(--dim);">
                    이 구간 진입 시 저항 없이 가격이 빠르게 밀릴 수 있습니다.
                </div>
            </div>
        """, unsafe_allow_html=True)

    # 사다리 맵 HTML 렌더링 (코드 생략 없이 유지)
