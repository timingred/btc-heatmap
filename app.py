...
if 'cvd_history' not in st.session_state: st.session_state.cvd_history = []
...
def fetch_data(coin, period_label):
    # 가격
    ...
    return {"price": px, "bn_liq": bn_res, "positions": hl_pos, "magnet": (potential_vol/1e6)}
...
data = fetch_data(coin, st.session_state.period)

if data:
    px, bn_orders, magnet_score = data['price'], data['bn_liq'], data['magnet']
    ...
    if isinstance(bn_orders, list):
        for o in bn_orders:
            ...
            if o_time > st.session_state.last_liq_time:
                ...
                # 숏 청산(SELL)은 가격 상승 요인(+), 롱 청산(BUY)은 가격 하락 요인(-)
                if o['side'] == 'SELL': current_delta += val
                else: current_delta -= val
                st.session_state.last_liq_time = max(st.session_state.last_liq_time, o_time)
    
    st.session_state.cvd_val += (current_delta / 1e6)
    st.session_state.cvd_history.append({"time": get_kst_now(), "cvd": st.session_state.cvd_val})
    if len(st.session_state.cvd_history) > 100: st.session_state.cvd_history.pop(0)

    # 상단 분석 카드 (CVD vs Magnet)
    a1, a2 = st.columns(2)
    with a1:
        cvd_color = "var(--green)" if current_delta >= 0 else "var(--red)"
        st.markdown(f"""
            <div class="analysis-card" style="border-top:4px solid {cvd_color};">
                <div style="color:var(--dim); font-size:11px;">CUMULATIVE LIQUIDATION DELTA (CVD)</div>
                <div style="font-size:32px; font-weight:900; color:{cvd_color}; margin:10px 0;">{st.session_state.cvd_val:+.2f}M</div>
                <div style="font-size:12px; color:#aaa;">청산 기반 시장 에너지 방향성</div>
                <div style="position:absolute; bottom:15px; font-size:11px; color:var(--dim);">최근 델타: {current_delta/1e6:+.2f}M</div>
            </div>
        """, unsafe_allow_html=True)
    with a2:
        s_tag, s_color = ("CRITICAL", "var(--red)") if magnet_score > 15 else (("WARNING", "var(--orange)") if magnet_score > 8 else ("STABLE", "#6b6b7b"))
        st.markdown(f"""
            <div class="analysis-card" style="border-top:4px solid {s_color};">
                <div style="color:var(--dim); font-size:11px;">LIQUIDATION MAGNET INTENSITY</div>
                <div style="font-size:32px; font-weight:900; color:{s_color}; margin:10px 0;">${magnet_score:.2f}M</div>
                <div style="margin-top:5px;"><span class="status-tag" style="background:{s_color};">{s_tag}</span></div>
                <div style="position:absolute; bottom:15px; font-size:11px; color:var(--dim);">현재가 근처 잠재적 청산 폭발력</div>
            </div>
        """, unsafe_allow_html=True)

    # CVD 차트 시각화
    cvd_df = pd.DataFrame(st.session_state.cvd_history)
    if not cvd_df.empty:
        st.area_chart(cvd_df.set_index("time"), height=150, use_container_width=True)
    ...
    whales = sorted(data['positions'], key=lambda x: x['posVal'], reverse=True)[:15]
    payload = json.dumps({"price": px, "map": ladder, "whales": whales, "maxV": max_v, "maxD": max_d, "coin": coin, "minV": min_val, "tL": sum(p['posVal'] for p in data['positions'] if p['isLong']), "tS": sum(p['posVal'] for p in data['positions'] if not p['isLong'])})
